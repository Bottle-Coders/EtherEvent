  ...
  async ({ connector }: { connector: Connector }) => {
    try {
      ...
      if (!provider) {
        ...
        console.log('Provider is not null or undefined.')
      }
      iExecDataProtector = new IExecDataProtector(provider as any)
      iExecWeb3Mail = new IExecWeb3mail(provider as any)
      console.log('[initSDK] iExecDataProtector', iExecDataProtector !== null)
      console.log('[initSDK] iExecWeb3Mail', iExecWeb3Mail !== null)
    } catch (error) {
